...
export default function Roadmap({ topics }: RoadmapProps) {
  ...
  // Collect all valid checkbox IDs from current topics to filter global state

  const { total, completed } = React.useMemo(() => {
    let total = 0
    let completed = 0

    const count = (items: CurriculumTopic[]) => {
      items.forEach(t => {
        t.checkboxes?.forEach(c => {
          total++
          if (completedCheckboxes[c.id]) completed++
        })
        if (t.subtopics) count(t.subtopics)
      })
    }
    count(topics)
            ...
            <AccordionTrigger className="hover:no-underline py-6">
              <div className="flex items-center gap-4 text-left">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 border-primary/20 text-sm font-bold">
                  {topic.checkboxes?.length || topic.subtopics.length}
                </div>
                              ...
                              {sub.checkboxes.map((check) => (
                                <div key={check.id} className="flex items-center space-x-2">
                                  <Checkbox
                                    id={check.id}
                                    checked={!!completedCheckboxes[check.id]}
                                    onCheckedChange={() => toggleCheckbox(sub.id, check.id)}
                                  />
